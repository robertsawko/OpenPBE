...
MOC::MOC(const dictionary &pbeProperties, const phaseModel &phase)
    ...
    forAll(classNumberDensity_, i) {
        classNumberDensity_.set(
            i,
            new volScalarField(IOobject("n" + std::to_string(i),
        ...
        xi_.emplace_back("xi" + std::to_string(i), deltaXi_ * (i + 1));
        Info << i << " has volume " << xi_[i].value() << " and diameter "
             << volumeToDiameter(xi_[i]).value() << endl;
    }
}
...
volScalarField MOC::coalescenceSourceTerm(label i) {
    ...
    volScalarField coalescenceField(
        ...
        dimensionedScalar(
            "Scoal", classNumberDensity_[i].dimensions() / dimTime, 0));
...
volScalarField MOC::breakupSourceTerm(label i) {
    ...
    volScalarField breakupField(
        ...
        // TODO Is the dimension correct?
        dimensionedScalar(
            "Sbr", classNumberDensity_[i].dimensions() / dimTime, 0.0));

    auto phaseSize = phase_.size();

    forAll(phase_, celli) {
        breakupField[celli] -= breakupCache_[i * phaseSize + celli] *
                               classNumberDensity_[i][celli];
        ...
        for (label j = i + 1; j < classNumberDensity_.size(); ++j) {
            ...
            breakupField[celli] +=
                daughterParticleDistribution_().beta(xi_[i].value(),
                                                     xi_[j].value()) *
...
void MOC::solveWithFVM(const PtrList<volScalarField> &S) {
    ...
    forAll(classNumberDensity_, k) {
        surfaceScalarField phi =
            fvc::interpolate(classVelocity_[k]) & mesh.Sf();
        fvScalarMatrix nEqn(
...
void MOC::solveWithMULES(const PtrList<volScalarField> &S) {
    ...
    forAll(classNumberDensity_, k) {
        ...
        phiNkCorrs.set(k,
                       new surfaceScalarField(
                           "phi" + Nk.name() + "Corr",
                           fvc::flux(phi, Nk, "div(phi," + Nk.name() + ')')));
    ...
    volScalarField volumeSum(
        IOobject("sumVolumeNk", mesh.time().timeName(), mesh),
        mesh,
...
const volScalarField MOC::d() const {
    ...
    volScalarField norm = 1.0 * classNumberDensity_[0];
