...
TEST(qmom, wheeler_test){
    ...
    // Analytical solution
    scalar w1 = 1 / (2 * sqrt(6) + pow(2, 1.5) * sqrt(3) + 12);
    scalar w2 = 1 / (12 - 2 * sqrt(6) - pow(2, 1.5) * sqrt(3));
    ...
    ASSERT_DOUBLE_EQ((p.first - abcissas).norm(), 0.);
    ASSERT_DOUBLE_EQ((p.second - weights).norm(), 0.);
